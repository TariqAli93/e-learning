<template>
  <div class="NotificationsCourses">
...
export default {
  ...
  methods: {
    ...
    async sendNotification() {
      ...
      try {
        const send = await this.$axios.post('studentCoursesNotification', {
          title: this.notificationsTitle,
          content: this.notificationsText,
          id: this.notificationsCourse,
        })
        ...
        this.$nuxt.$loading.finish()
         this.$toast.error('لم يتم ارسال الاشعار! لا يوجد طلاب في هذا الكورس', {
          position: 'top-center',
          duration: 3000,
        })
        console.error(error)
