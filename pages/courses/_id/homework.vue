      ...
      <v-toolbar flat color="primary" class="shadow-1 radius-1 mb-10">
        <div class="d-flex align-center justify-evenly" style="width: 100%">
          <v-toolbar-title style="flex: 1 1 auto">
            {{ courseName }} <v-icon>arrow_left</v-icon> الواجبات
          </v-toolbar-title>
          <v-text-field
            v-model="search"
            color="text"
      ...
      <v-row v-if="courseLibraries.length > 0">
        ...
        >
          <v-card color="primary" class="shadow-1 radius-1 pa-5">
            ...
            />
            <v-card-title>{{ file.homeworkTitle }}</v-card-title>
            <v-card-text>{{ file.homeworkSubTitle }}</v-card-text>
            ...
            <v-card-actions>
              <v-btn
                color="success"
                text
                :href="$axios.defaults.baseURL + file.homeworkPath "
              >
                تحميل
              </v-btn>
              <v-spacer />
  ...
  methods: {
    ...
    async removeFile(item) {
      if (confirm('هل تريد حذف الملف')) {
        try {
          const remove = await this.$axios.delete(`library/${item.idHomework}`)
    ...
    async uploadFile() {
      if (this.$refs.uploadNewFileRef.validate()) {
        try {
          ...
          const addLibrary = await this.$axios.post('addHomework', {
            homeworkTitle: this.fileTitle,
            homeworkSubTitle: this.fileSubTitle,
            homeworkPath: this.fileUploaded,
            courseId: this.$route.params.id * 1,
            createdBy: this.$auth.user.idUser * 1,
            homeworkType: this.uploadType * 1,
          })
    ...
    async courseDetails() {
      try {
        ...
        this.courseLibraries = course.data.Homework
        console.log(course)
      } catch (error) {
        console.error(error.response)
      }
